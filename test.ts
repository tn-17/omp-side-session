...
test("registers /side with subcommand suggestions without replacing OMP's built-in /btw", () => {
	...
	expect(complete("r")?.map(item => item.label)).toEqual(["recover"]);
...
test("agent-end maintenance deletes a child after its pane closes manually", async () => {
	...
	try {
		...
		expect(await pathExists(sessionFile)).toBe(false);
		expect(notifications).toContain("Deleted Side 1 after its pane closed.");
		const latest = mainBranch.at(-1)?.data as { sides?: unknown[] };
		expect(latest.sides).toEqual([]);
	} finally {
		if (previousHerdr === undefined) delete process.env.HERDR_ENV;
		else process.env.HERDR_ENV = previousHerdr;
